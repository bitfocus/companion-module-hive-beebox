...
class HiveBeebladeInstance extends InstanceBase {
	...
	setTimecodeEnable(enable, layer) {
		if (layer > 0 && layer) {
			layer = layer - 1
			if (this.localSVPatch.connected)
				this.localSVPatch.UpdatePatchJSON('/Timecode Cue List', [
					{ op: 'replace', path: '/layers/' + layer + '/useCueList', value: enable ? 1 : 0 },
				])
		}
	}
	...
	setViosoEnable(enable) {
		if (this.localSVPatch.connected)
			this.localSVPatch.UpdatePatchJSON('/Vioso WB Settings', [
				{ op: 'replace', path: '/enabled', value: enable ? 1 : 0 },
			])
	}
	...
	setViosoCalibrationEnable(enable) {
		if (this.localSVPatch.connected)
			this.localSVPatch.UpdatePatchJSON('/Vioso WB Settings', [
				{ op: 'replace', path: '/calibrationMode', value: enable ? 1 : 0 },
			])
	}

	setScreenberryEnable(enable) {
		if (this.localSVPatch.connected)
			this.localSVPatch.UpdatePatchJSON('/Screenberry WB Settings', [
				{ op: 'replace', path: '/enabled', value: enable ? 1 : 0 },
			])
	}
	...
	calculatePlayheadTime(playlist, currentRowIndex, timeincurrent) {
		if (currentRowIndex >= 0 && currentRowIndex < playlist.list.length) {
			...
			for (var i = 0; i < currentRowIndex; i++) {
				...
				if (playlist.transitionDuration > 0.0) sumPreviousMediaLength -= playlist.transitionDuration
			}
	...
	GetMediaFileDuration(playlist, iList) {
		if (playlist.list[iList].followAction >= 8 && playlist.list[iList].followAction < 15) {
	...
	CalculatePlayListDuration(playlist) {
		...
		playListAutoTransitionMode =
			typeof playlist.transitionDuration === 'undefined' || playlist.transitionDuration === 0.0 ? false : true

		if (playListAutoTransitionMode) duration -= (playlist.list.length - 1) * playlist.transitionDuration
